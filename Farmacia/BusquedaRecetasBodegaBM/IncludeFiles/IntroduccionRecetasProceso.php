...
if($test["IdFarmacia"]!=4 and $_SESSION["TipoFarmacia"]==2){
	echo "NO~";
 	...
 	$Cierre=$proceso->Cierre($test["FechaConsulta"]);
 	$CierreMes=$proceso->CierreMes($test["FechaConsulta"]);
   ...
   if(($respCierre[0]!=NULL and $respCierre[0]!='') || ($respCierreMes[0]!=NULL and $respCierreMes[0]!='')){
	...
	$resp=$proceso->ObtenerMedicinaIntroducida($IdReceta);
			$proceso->CambiarEstado($IdReceta);
	...
	$Datos=$proceso->ObtenerDatosGenerales($IdReceta);
		$NombreMedico="<a href='#' onClick='javascript:Correcciones(\"NombreMedico\");'>".strtoupper($Datos["NombreEmpleado"])."</a>";
		
			if($Datos["Origen"]!=NULL or $Datos["Origen"]!=""){$Origen=$Datos["Origen"]." -> ";}else{$Origen="";}
...
case 14:
	...
	if(isset($_GET["IdArea"])){
		...
		if($IdReceta=='' or $IdReceta==NULL){
			...
			$salida=$proceso->ActualizarArea($IdArea,$IdReceta,$IdFarmacia);
		}
	...
	if(isset($_GET["IdAreaOrigen"])){
		...
		//if($IdArea!=$IdAreaOriginal){
		   if($IdReceta=='' or $IdReceta==NULL){
			...
			$salida=$proceso->ActualizarAreaOrigen($IdArea,$IdReceta);
		   }
		//}
	}//Actualizacion de Area
...
case 16:
	...
	$IdHistorialClinico=$_GET["IdHistorialClinico"];
echo $proceso->CambiarFecha($IdReceta,$FechaNueva,$IdHistorialClinico);
...
case 18:
	...
	$IdHistorialClinico=$_GET["IdHistorialClinico"];
echo $proceso->CambiarExpediente($IdNumeroExp,$IdHistorialClinico);
break;
}//Fin de switch

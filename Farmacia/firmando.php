...
$contra=md5($contra);
$DB=new BaseDeDato("localhost","5432","siap_ultimate","postgres","admin");//declarar el objeto de la clase base de dato
$q="SELECT * FROM fos_user_user where username='$usuario' and password='$contra'";
$result3=$DB->Consultas($q);//llamar a la funcion de la base de dato que realiza las consulta
...
$NombreEstablecimiento1=pg_fetch_array($DB->Consultas($query));
$Area=pg_fetch_array($DB->Consultas($query2));

$DB->Consultas("update farm_usuarios set Conectado='S' where IdPersonal=".$id);
$DB->Consultas("update farm_usuarios set UltimaConexion=now() where IdPersonal=".$id);

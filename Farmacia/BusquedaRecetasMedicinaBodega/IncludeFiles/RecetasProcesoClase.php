...
class RecetasProceso{
	...
	function Combo($Tipo,$TipoFarmacia){
		switch($Tipo){
			case 1:
				/*COMBO DE FARMACIA*/
				if($TipoFarmacia==1){
			...
			case 2:
				...
				$query="select IdArea,Area
						from mnt_areafarmacia
						where IdArea=12 and Habilitado ='S' ";
				}
				$resp=mysql_query($query);
				$combo='';
				while($row=mysql_fetch_array($resp)){
...
/* FIN HISTORIAL CLINICO */
	...
	function ObtenerRecetas($IdMedicina,$IdFarmacia,$IdArea,$IdSubEspecialidad,$IdEmpleado,$FechaInicial,$FechaFinal,$TipoFarmacia){
	...
	function ActualizarInventario($IdMedicina,$IdMedicinaRecetada,$Cantidad,$IdArea,$Fecha){
		...
		//****** Si la existencia del lote es menor a lo que se descargara, se debe utilizar el segundo lote...
			...
			//Se cierra el lote con existencia = 0
				$actualiza="update farm_entregamedicamento set Existencia='0' where IdMedicina='$IdMedicina' and IdLote='$IdLote'";
				mysql_query($actualiza);

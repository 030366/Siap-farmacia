...
//Obtener Medicina Introducida
    ...
    function ObtenerDatosGenerales($IdReceta,$IdEstablecimiento,$IdModalidad) {
        ...
        $query="select farm_recetas.IdArea,sec_historial_clinico.IdNumeroExp,
                        (select Area from mnt_areafarmacia where IdArea=farm_recetas.IdArea) as Area, 
                        NombreEmpleado, mnt_subservicio.NombreSubServicio,( select NombreServicio from mnt_servicio ms inner join mnt_servicioxestablecimiento mse on ms.IdServicio=mse.IdServicio inner join mnt_subservicioxestablecimiento msse on msse.IdServicioxEstablecimiento = mse.IdServicioxEstablecimiento inner join mnt_subservicio mss on mss.IdSubServicio = msse.IdSubServicio
                        where msse.IdEstablecimiento=$IdEstablecimiento
                        and msse.IdModalidad=$IdModalidad and mss.IdSubServicio = sec_historial_clinico.IdSubServicio )  as Origen,
...
//actualizaicon de area
    ...
    function CambiarExpediente($IdNumeroExp, $IdHistorialClinico) {
